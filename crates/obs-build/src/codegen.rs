...
pub(crate) fn render_lints(events: &[EventDecl], event_prefix: &str) -> String {
    ...
    out.push_str("// @generated by obs-build. DO NOT EDIT.\n");
    out.push_str(
        "// File: lints.rs — const-eval lint asserts (spec 12 § 3.4 / spec 94 § 2.3 P1-A).\n// A \
...
"#,
    ...
    if decl.fields.is_empty() {
        let msg = format!(
            "obs L009: event `{rust_name}` has no fields\nnote: empty events make analytics joins \
    ...
    // Per-field L001/L002/L004/L007/L012 + per-event L003/L006.
    let tier = decl.event.tier.unwrap_or(Tier::Log);
    for f in &decl.fields {
        let kind = f.options.kind.unwrap_or(FieldKind::Attribute);
...
"#,
        ...
        if matches!(kind, FieldKind::Measurement) && f.options.metric.is_none() {
            // Avoid literal `{` / `}` in the panic string — the
            // `panic!(literal)` macro re-parses it as a format string.
            let msg = format!(
                "obs L004: field `{}` is MEASUREMENT without a metric kind\nnote: MEASUREMENT \
                 fields must declare a metric kind (counter / gauge / histogram) so the OTLP \
                 metric sink can dispatch correctly.\nhelp: annotate the proto field with a \
                 metric option such as kind=METRIC_KIND_COUNTER and a unit string.",
                f.name
            );
            s.push_str(&format!(
                "const _: () = ::std::panic!({msg});\n",
        ...
        if matches!(tier, Tier::Audit)
            && matches!(classification, Classification::Pii | Classification::Secret)
        {
            ...
            let msg = format!(
                "obs L006: AUDIT-tier event must not carry `{}` field `{}`\nnote: AUDIT events \
        ...
        {
            let msg = format!(
                "obs L012: field `{}` shadows envelope-reserved name\nnote: `{}` is one of the \
                 obs envelope's first-class fields. A payload field by the same name would clash \
                 on the analytics surface.\nhelp: rename the field; if the intent was to project \
                 onto the envelope slot, set the appropriate kind (e.g. `kind: TRACE_ID`).",
                f.name, f.name
            );
            s.push_str(&format!(
                "const _: () = ::std::panic!({msg});\n",
...
/// L013 — runtime startup check that no two `full_name`s share a
/// schema_hash within this codegen unit. Spec 94 § 2.3 / P1-A.
///
/// Implemented as a const-eval `assert!` that pairs every event with
/// every other event in declaration order. Compile time is `O(N^2)`
/// in the const-eval engine but N is workspace-bounded (typical < 200
/// events) and the alternative — runtime detection in `registry/mod.rs`
/// — fires after the binary boots, which is too late to be useful.
fn render_l013(events: &[EventDecl]) -> String {
    if events.len() < 2 {
        return String::new();
    }
    let mut s = String::new();
    s.push_str("\n// L013: schema_hash uniqueness across this codegen unit. Spec 94 § 2.3.\n");
    for (i, a) in events.iter().enumerate() {
        for b in events.iter().skip(i + 1) {
            let hash_a = a.schema_hash();
            let hash_b = b.schema_hash();
            if hash_a == hash_b {
                let msg = format!(
                    "obs L013: schema_hash collision: `{}` and `{}` both hash to {:#018x}.\nhelp: \
                     rename one event so the canonical descriptor differs (any field rename / \
                     reorder will do).",
                    a.full_name, b.full_name, hash_a
                );
                s.push_str(&format!(
                    "const _: () = ::std::panic!({msg});\n",
                    msg = rust_str_lit(&msg),
