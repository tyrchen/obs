...
## 3. `obs::Instrumented<F>` — async scope and observer adapter
...
`tracing-futures::Instrument`. It carries **two** kinds of context
across awaits and into spawned tasks:

1. **Scope frame** — the `obs::scope!` field allowlist + tail-on-error
   buffer.
2. **Observer override** — a per-task `Arc<dyn Observer>` for the
   multi-tenant / per-task debug-capture cases described in
   [11-runtime-core.md § 3.1](./11-runtime-core.md#31-the-three-tiers-and-what-each-is-for).

```rust
use obs::{Instrument, WithObserver};       // traits
...
// Carry only an observer override (multi-tenant):
tokio::spawn(
    handle_tenant_request(req)
        .with_observer(observer_for_tenant(tenant_id)),
);

// Carry both — composable, in either order:
tokio::spawn(
    handle_tenant_request(req)
        .instrument(obs::scope!(trace_id = req.id.clone()))
        .with_observer(observer_for_tenant(tenant_id)),
);
```

`Instrumented<F>`'s `Future::poll` enters the scope and binds the
observer task-local before delegating to the inner future, then
exits both on return; both guards live inside the `Instrumented`
value, so cancellation drops them correctly. Implementation:

```rust
impl<F: Future> Future for Instrumented<F> {
    type Output = F::Output;
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<F::Output> {
        // SAFETY: structural pin — projection generated by `pin-project-lite`.
        let this = self.project();
        let _scope_guard = this.scope.as_ref().map(|s| s.enter());
        match this.observer.as_ref() {
            Some(o) => OBSERVER_TASK.sync_scope(o.clone(), || this.inner.poll(cx)),
...
Both `instrument(...)` and `.with_observer(...)` may be called on the
same future; internally they populate orthogonal slots on the same
`Instrumented<F>` wrapper, so there is no nested wrapping cost.
...
`Instrumented<F>` and the `WithObserver` extension trait are in
`obs-sdk`, gated by no feature; library crates get them free.

### 3.1 Why per-task observer override is a tier above per-thread

`tokio::spawn`'d futures may run on **any** worker thread, and a
single future may suspend on one thread and resume on another. A
thread-local observer override therefore **leaks** into whichever
task happens to run on that thread next, and **disappears** when the
task migrates. Per-task storage (via `tokio::task_local!`) follows
the task across thread migration, which is the only correct
semantics for async multi-tenant code. The cost is one
`task_local::sync_scope` per `poll`; benchmarks show ≤ 30 ns
overhead.

This is the gap that `tracing` partially solves with `tracing-futures::Instrument`:
in tracing, the Span (which carries the dispatcher reference) is
captured at `.instrument()` time and re-entered on each poll. We
follow the same pattern explicitly for the observer, so the user
mental model is "use `.instrument()` *and/or* `.with_observer()` on
any future that should keep its context across awaits or spawns."
...
#### Mapping to envelope labels (NOT tracing fields)

EnvFilter's `[field=value]` clause matches against `tracing::Field`
values, which obs does not have. In `obs::Filter` the same syntactic
clause matches against **envelope `labels`** (the flat
`BTreeMap<&'static str, Cow<'static, str>>` populated by
`EventSchema::project`). The semantic shift is deliberate:

- Only fields declared with `kind: LABEL` reach `env.labels`, so
  the matchable set is bounded at compile time.
- `kind: ATTRIBUTE` field values live in the typed payload, which
  the filter pipeline does not decode (decoding per-emit would
  defeat the cardinality of the filter check).
- This means `[user_id=42]` (where `user_id` is ATTRIBUTE per L002)
  matches **nothing**, by design. `obs lint` warns on filter
  expressions that reference non-LABEL fields so operators don't
  silently get empty results.

Performance: directives are bucketed by `full_name` per § 7.0, so
the per-emit cost is one HashMap probe + one `BTreeMap::get` per
directive applicable to this event — typically zero or one
directives, never the full directive list.
